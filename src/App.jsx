...
// ─── DEDUCTION DEFAULTS (editable by user) ───────────────────────────────────
const DEFAULT_TAX_SETTINGS = {
  sss:  { baseContrib:180, stepAmount:22.5, minSalary:4250, maxSalary:29750, maxContrib:1350 },
  phic: { rate:0.05, minSalary:10000, maxSalary:100000 },
  hdmf: { rateLow:0.01, rateHigh:0.02, threshold:1500, maxContrib:200 },
  bir: [
    { min:0,       max:20833,   base:0,      rate:0    },
    { min:20833,   max:33333,   base:0,      rate:0.20 },
    { min:33333,   max:66667,   base:2500,   rate:0.25 },
    { min:66667,   max:166667,  base:10833,  rate:0.30 },
    { min:166667,  max:666667,  base:40833,  rate:0.32 },
    { min:666667,  max:9999999, base:200833, rate:0.35 },
  ],
};
function computeSSS(s,cfg){
  const salary=Math.min(Math.max(s,cfg.minSalary),cfg.maxSalary);
  const step=Math.round((salary-cfg.minSalary)/500);
  return Math.min(Math.round((cfg.baseContrib+step*cfg.stepAmount)*100)/100,cfg.maxContrib);
}
function computePhilHealth(s,cfg){
  const msc=Math.min(Math.max(s,cfg.minSalary),cfg.maxSalary);
  return Math.round(msc*(cfg.rate/2)*100)/100;
}
function computeHDMF(s,cfg){
  if(s<=cfg.threshold)return Math.round(s*cfg.rateLow*100)/100;
  return Math.min(Math.round(s*cfg.rateHigh*100)/100,cfg.maxContrib);
}
function computeWithholdingTax(s,ts){
  const sss=computeSSS(s,ts.sss),phic=computePhilHealth(s,ts.phic),hdmf=computeHDMF(s,ts.hdmf);
  const taxable=s-sss-phic-hdmf;
  let tax=0;
  for(const b of ts.bir){
    if(taxable>b.min&&taxable<=b.max){ tax=b.base+(taxable-b.min)*b.rate; break; }
  }
...
const INCOME_TYPES=[
  {v:"salary",l:"💼 Salary"},{v:"freelance",l:"💻 Freelance / Sideline"},
...
const INIT_BILLS=[
  ...
  {id:"b4",name:"Condo Dues",amount:3500,dueDay:15,paid:false,cat:"home"},
...
const INIT_BUDGETS=[
  {id:"bg1",cat:"food",label:"Food & Groceries",icon:"🛒",limit:8000},
  {id:"bg2",cat:"dining",label:"Dining Out",icon:"🍽️",limit:5000},
  {id:"bg3",cat:"transport",label:"Transport / Fuel",icon:"🚗",limit:4000},
  {id:"bg4",cat:"travel",label:"Travel & Vacations",icon:"✈️",limit:10000},
  {id:"bg5",cat:"entertainment",label:"Entertainment",icon:"🎬",limit:3000},
  {id:"bg6",cat:"shopping",label:"Shopping",icon:"🛍️",limit:6000},
  {id:"bg7",cat:"fitness",label:"Gym & Fitness",icon:"💪",limit:5000},
  {id:"bg8",cat:"date",label:"Date Nights",icon:"🌙",limit:4000},
  {id:"bg9",cat:"home",label:"Home Improvement",icon:"🔧",limit:5000},
  {id:"bg10",cat:"load",label:"Mobile / Internet",icon:"📱",limit:2500},
];
...
const pct2=n=>+(n*100).toFixed(4);
const dec2=n=>+(n/100).toFixed(6);
...
function Modal({title,onClose,children,wide=false}){
  return(
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.55)",zIndex:300,display:"flex",alignItems:"center",justifyContent:"center",padding:16}} onClick={onClose}>
      <div style={{background:"#F5F0E8",borderRadius:16,padding:26,width:"100%",maxWidth:wide?700:440,border:"1px solid #C8C2B6",maxHeight:"92vh",overflowY:"auto",boxShadow:"0 24px 60px rgba(0,0,0,.18)"}} onClick={e=>e.stopPropagation()}>
...
function MatchPreview({amount,type,earner,sources}){
  if(!amount||+amount<=0)return null;
  const rt=type||"salary",re=earner||"You";
  const hit=sources.find(s=>s.type===rt&&(s.earner===re||s.earner==="Joint"||re==="Joint")&&Math.abs(+amount-s.expectedAmt)/(s.expectedAmt||1)<=0.10);
  if(hit)return <div style={{background:"#0D9E8012",border:"1px solid #0D9E8040",borderRadius:8,padding:"9px 13px",fontSize:12,color:"#0D6B55",marginBottom:12}}>✅ Matches <strong>{hit.desc}</strong> — {pesoF(hit.expectedAmt)} expected</div>;
  if(rt!=="salary")return <div style={{background:"#C97A0A12",border:"1px solid #C97A0A30",borderRadius:8,padding:"9px 13px",fontSize:12,color:"#7A4A00",marginBottom:12}}>⚠️ No matching income source found. Add one in Income Analysis.</div>;
  return null;
}
...
export default function App(){
  ...
  const [incomeSources,setIncomeSources] = useState(INIT_SOURCES);
  const [receivedIncome,setReceivedIncome]=useState(INIT_RECEIVED);
  ...
  // ── CLOUD SYNC (loop-safe) ────────────────────────────────────────────────
  // The bug before: onSnapshot fires → we set state → save-effect fires →
  // writes to Firestore → onSnapshot fires again → infinite loop / overwrites.
  // Fix: isRemote ref is true while we apply a cloud snapshot, so the
  // save-effect ignores that state change entirely.
  ...
  useEffect(()=>{
    ...
    const unsub=onSnapshot(ref,
      snap=>{
        ...
        if(snap.exists()){
          const d=snap.data();
          if(d.incomeSources)  setIncomeSources(d.incomeSources);
          if(d.receivedIncome) setReceivedIncome(d.receivedIncome);
          if(d.expenses)       setExpenses(d.expenses);
          if(d.bills)          setBills(d.bills);
          if(d.cards)          setCards(d.cards);
          if(d.budgets)        setBudgets(d.budgets);
          if(d.partnerNames)   setPartnerNames(d.partnerNames);
          if(d.salaries)       setSalaries(d.salaries);
          if(d.taxSettings)    setTaxSettings(d.taxSettings);
        }
        cloudReady.current=true;
        setSyncStatus("synced");
        // Release flag after React flushes the state updates above
  ...
  // Only save when the user changed something locally (not when we got a remote update)
  useEffect(()=>{
    if(!cloudReady.current||isRemote.current)return;
    setSyncStatus("saving");
    const t=setTimeout(()=>{
      setDoc(doc(db,"pesotrack",HOUSEHOLD_ID),
        {incomeSources,receivedIncome,expenses,bills,cards,budgets,partnerNames,salaries,taxSettings,updatedAt:new Date().toISOString()},
        {merge:true}
      ).then(()=>setSyncStatus("synced"))
       .catch(err=>{console.error("Firestore write:",err);setSyncStatus("error");});
    },900);
    return()=>clearTimeout(t);
  },[incomeSources,receivedIncome,expenses,bills,cards,budgets,partnerNames,salaries,taxSettings]);
  // ─────────────────────────────────────────────────────────────────────────
  ...
  const totalExpOther  =useMemo(()=>incomeSources.reduce((s,x)=>s+x.expectedAmt,0),[incomeSources]);
  const matchSource    =useCallback(rx=>incomeSources.find(s=>s.type===rx.type&&(s.earner===rx.earner||s.earner==="Joint"||rx.earner==="Joint")&&Math.abs(rx.amount-s.expectedAmt)/(s.expectedAmt||1)<=0.10)||null,[incomeSources]);
  const dedYou  =useMemo(()=>computeWithholdingTax(salaries.you,taxSettings),[salaries.you,taxSettings]);
  const dedPart =useMemo(()=>computeWithholdingTax(salaries.partner,taxSettings),[salaries.partner,taxSettings]);
  ...
  const addBudget=()=>{
    ...
    const ci=catInfo(form.cat);
    setBudgets(x=>[...x,{id:"bg"+Date.now(),cat:form.cat,label:ci.l,icon:ci.i,limit:+form.limit}]);
    closeModal();
  };
  const toggleBill=id=>setBills(x=>x.map(b=>b.id===id?{...b,paid:!b.paid}:b));
  const eColor=e=>e==="Joint"?"#38BDF8":e==="Partner"?"#F472B6":"#0D9E80";
  ...
  const CSS=`
    ...
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    input::placeholder{color:#A89E94} select option{background:#F5F0E8}
    ...
    <>
          ...
          <div style={{padding:"0 18px 22px"}}>
            <div style={{display:"flex",alignItems:"center",gap:9}}>
              ...
              <div>
                ...
                <div style={{fontSize:10,color:"#78716C"}}>DINK Edition · 2025</div>
              </div>
            </div>
          </div>
          ...
          {tab==="overview"&&(
            <div>
              <div style={{marginBottom:24}}>
                ...
                <p style={{color:"#78716C",fontSize:13}}>Your household financial snapshot</p>
              </div>
              ...
              {/* Headroom */}
              <div style={{background:"linear-gradient(135deg,#EAE4D8,#E4DED2)",borderRadius:16,padding:"22px 26px",border:"1px solid #D0CABC",marginBottom:20}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:16,marginBottom:20}}>
                  <div>
                    ...
                    <div style={{fontSize:12,color:"#78716C",marginTop:6}}>Net cash + available credit</div>
                  </div>
                  <div>
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginBottom:18}}>
                  <div style={{background:"rgba(255,255,255,.55)",borderRadius:12,padding:"14px 16px",border:"1px solid #D0CABC"}}>
                    ...
                    <div style={{fontFamily:"Plus Jakarta Sans",fontWeight:800,fontSize:22,color:netCash>=0?"#0D9E80":"#D94F4F",marginBottom:10}}>{netCash<0&&"−"}{peso(netCash)}</div>
                  ...
                  <div style={{background:"rgba(255,255,255,.55)",borderRadius:12,padding:"14px 16px",border:"1px solid #D0CABC"}}>
                    ...
                    <div style={{fontFamily:"Plus Jakarta Sans",fontWeight:800,fontSize:22,color:"#4F54D4",marginBottom:10}}>{peso(availableCredit)}</div>
                    {cards.map(c=>{
                      ...
                      return(
                        <div key={c.id} style={{marginBottom:8}}>
                          <div style={{display:"flex",justifyContent:"space-between",marginBottom:3}}>
                            ...
                            <span style={{fontSize:11,fontWeight:600,color:c.color}}>{peso(Math.max(0,c.limit-used))} free</span>
                          </div>
                ...
                <div style={{height:8,background:"#D8D2C8",borderRadius:4,overflow:"hidden",display:"flex",marginBottom:6}}>
                  <div style={{height:"100%",width:`${Math.max(0,Math.min((paidBills/totalIncome)*100,100))}%`,background:"#C97A0A"}}/>
                  <div style={{height:"100%",width:`${Math.max(0,Math.min((totalExpenses/totalIncome)*100,100))}%`,background:"#D94F4F"}}/>
                    ...
                    return(
                      <div key={c.id} style={{marginBottom:12}}>
                        <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
                          <div><span style={{fontSize:13,fontWeight:600}}>{c.name}</span><span style={{fontSize:10,color:"#78716C",marginLeft:6}}>Due {c.dueDay}</span></div>
                          <div><span style={{fontSize:14,fontWeight:700,color:sc}}>{peso(total)}</span>{newChg>0&&<span style={{fontSize:10,color:"#C97A0A",marginLeft:5}}>+{peso(newChg)}</span>}</div>
                        </div>
                  ...
                  {upcoming.slice(0,7).map(u=>(
                    <div key={u.key} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"7px 0",borderBottom:"1px solid #DDD8CE55"}}>
                      <div style={{display:"flex",alignItems:"center",gap:8}}>
                        ...
                        <div><div style={{fontSize:12,fontWeight:600}}>{u.name}</div><div style={{fontSize:10,color:"#78716C"}}>Due {u.dueDay}</div></div>
                      </div>
            ...
            <div>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:22}}>
                <div>
                  ...
                  <p style={{color:"#78716C",fontSize:13}}>Record income received & expenses.</p>
                </div>
                <div style={{display:"flex",gap:8}}>
                  <Btn variant="ghost" onClick={()=>setModal("addIncome")}>+ Income Received</Btn>
              ...
              <div style={{marginBottom:24}}>
                ...
                {receivedIncome.length===0&&<div style={{background:"#FDFAF5",borderRadius:11,border:"1px dashed #DDD8CE",padding:20,textAlign:"center",color:"#78716C",fontSize:13}}>No income recorded yet.</div>}
                    ...
                    return(
                      <div key={i.id} style={{display:"flex",alignItems:"flex-start",gap:12,padding:"13px 18px",background:"#FDFAF5",borderRadius:11,border:"1px solid",borderColor:matched?"#0D9E8055":"#DDD8CE"}}>
                        ...
                        <div style={{flex:1,minWidth:0}}>
                          ...
                          {matched&&<div style={{fontSize:10,color:"#0D9E80",marginTop:3}}>✅ Matches <strong>{matched.desc}</strong> — {pesoF(matched.expectedAmt)} expected · {Math.round(Math.abs(i.amount-matched.expectedAmt)/(matched.expectedAmt||1)*100)}% variance</div>}
                          {!matched&&i.type!=="salary"&&<div style={{fontSize:10,color:"#C97A0A",marginTop:3}}>⚠️ No matching income source in Income Analysis</div>}
                        </div>
                        <Tag color={eColor(i.earner)}>{i.earner}</Tag>
                  ...
                  {expenses.map(e=>{
                    ...
                    return(
                      <div key={e.id} style={{display:"flex",alignItems:"center",gap:12,padding:"13px 18px",background:"#FDFAF5",borderRadius:11,border:"1px solid #DDD8CE"}}>
                        ...
                        <Tag color={eColor(e.earner)}>{e.earner}</Tag>
            ...
            <div>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:18}}>
                <div>
                  ...
                  <p style={{color:"#78716C",fontSize:13}}><span style={{color:"#0D9E80"}}>{peso(paidBills)} paid</span> · <span style={{color:"#C97A0A"}}>{peso(unpaidBills)} remaining</span></p>
                </div>
                  ...
                  return(
                    <div key={b.id} style={{display:"flex",alignItems:"center",gap:13,padding:"14px 18px",background:"#FDFAF5",borderRadius:11,border:"1px solid",borderColor:b.paid?"#0D9E8033":overdue?"#D94F4F55":soon?"#C97A0A33":"#DDD8CE"}}>
                      ...
                      <div style={{flex:1}}>
                        ...
                        <div style={{fontSize:11,color:"#78716C"}}>Due {b.dueDay} · {ci.l}</div>
                      </div>
                    ...
                    <div key={c.id} style={{background:"#FDFAF5",borderRadius:16,border:`1px solid ${c.color}44`,overflow:"hidden"}}>
                      <div style={{padding:"20px 24px",borderBottom:"1px solid #DDD8CE",display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                        ...
                        <div style={{textAlign:"right"}}>
                          ...
                          <div style={{fontFamily:"Plus Jakarta Sans",fontWeight:800,fontSize:28,color:sc}}>{peso(totalOwed)}</div>
                        </div>
                      </div>
                      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",padding:"14px 24px",gap:12,borderBottom:"1px solid #DDD8CE"}}>
                        {[{l:"Statement Bal.",v:peso(c.statementBalance),c:"#1C1917"},{l:"New Charges",v:peso(newChg),c:"#C97A0A"},{l:"Min. Pay (3%)",v:peso(minPay),c:"#D94F4F"},{l:"Cut-off Day",v:`Day ${c.cutDay}`,c:"#4F54D4"}].map(s=>(
                          <div key={s.l}>
                      ...
                      <div style={{padding:"14px 24px",display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:10}}>
                        <div style={{fontSize:12,color:"#78716C"}}>Payment due: <span style={{color:"#C97A0A",fontWeight:600}}>Day {c.dueDay}</span></div>
                        <div style={{display:"flex",gap:8}}>
            ...
            <div>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:22}}>
                <div>
                  ...
                  <p style={{color:"#78716C",fontSize:13}}>Set monthly limits. Edit or delete any category.</p>
                </div>
                <Btn onClick={()=>setModal("addBudget")}>+ Add Category</Btn>
              </div>
              <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(270px,1fr))",gap:14}}>
                {budgets.map(b=>{
                  const spent=spendByCat[b.cat]||0,pct=(spent/b.limit)*100,isOver=spent>b.limit;
                  const ringColor=isOver?"#D94F4F":CAT_COLORS[b.cat]||"#0D9E80";
                  return(
                    <div key={b.id} style={{background:"#FDFAF5",borderRadius:14,padding:18,border:"1px solid",borderColor:isOver?"#D94F4F44":"#DDD8CE"}}>
                      <div style={{display:"flex",alignItems:"center",gap:14}}>
                        ...
                        <div style={{flex:1,minWidth:0}}>
                          <div style={{fontSize:14,fontWeight:700,marginBottom:3}}>{b.label}</div>
                          <div style={{fontSize:13,color:isOver?"#D94F4F":"#0D9E80",fontWeight:600}}>{peso(spent)} <span style={{color:"#78716C",fontWeight:400}}>/ </span></div>
                          {/* Editable limit inline */}
                          <div style={{display:"flex",alignItems:"center",gap:5,marginTop:4}}>
                            <span style={{fontSize:10,color:"#78716C"}}>Limit ₱</span>
                            <input type="number" value={b.limit}
                              onChange={e=>setBudgets(x=>x.map(bg=>bg.id===b.id?{...bg,limit:+e.target.value||0}:bg))}
                              style={{width:80,background:"#EDE8DE",border:"1px solid #C8C2B6",borderRadius:6,padding:"4px 8px",color:"#1C1917",fontSize:12,outline:"none",fontFamily:"inherit"}}/>
                          </div>
                          <div style={{fontSize:10,color:isOver?"#D94F4F":"#78716C",marginTop:3}}>{isOver?`Over by ${peso(spent-b.limit)}`:`${peso(b.limit-spent)} remaining`}</div>
                        </div>
                        <button onClick={()=>setBudgets(x=>x.filter(bg=>bg.id!==b.id))} style={{background:"none",border:"none",color:"#C8C2B6",cursor:"pointer",fontSize:16,padding:"0 2px",alignSelf:"flex-start"}}>×</button>
                      </div>
                    </div>
                  );
                })}
          ...
          {tab==="analysis"&&(
            <div>
              <div style={{marginBottom:22}}>
                ...
                <p style={{color:"#78716C",fontSize:13}}>SSS, PhilHealth, Pag-IBIG & BIR — TRAIN Law. Edit rates directly.</p>
              </div>
              ...
              <div style={{background:"#FDFAF5",borderRadius:13,padding:"16px 20px",border:"1px solid #DDD8CE",marginBottom:18,display:"flex",gap:16,flexWrap:"wrap",alignItems:"center"}}>
                <span style={{fontSize:12,color:"#78716C",flexShrink:0}}>Earner names:</span>
                {["you","partner"].map(k=>(
                  <div key={k} style={{display:"flex",gap:8,alignItems:"center"}}>
                    <span style={{fontSize:12,color:k==="you"?"#0D9E80":"#F472B6"}}>{k==="you"?"Earner 1":"Earner 2"}:</span>
                    <input value={partnerNames[k]} onChange={e=>setPartnerNames(x=>({...x,[k]:e.target.value}))} style={{width:110,background:"#EDE8DE",border:"1px solid #C8C2B6",borderRadius:7,padding:"6px 10px",color:"#1C1917",fontSize:13,outline:"none",fontFamily:"inherit"}}/>
                  </div>
                ))}
              </div>
              ...
              <div style={{background:"linear-gradient(135deg,#EDE7DB,#E4DED2)",borderRadius:13,padding:"20px 24px",border:"1px solid #C0BAB0",marginBottom:18,display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(130px,1fr))",gap:16}}>
                {[
                  ...
                  {l:"Other (Expected)",v:pesoF(totalExpOther),c:"#C97A0A"},
              ...
              {/* Per-earner cards */}
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginBottom:18}}>
                {[{name:partnerNames.you,salary:salaries.you,ded:dedYou,color:"#0D9E80",key:"you"},{name:partnerNames.partner,salary:salaries.partner,ded:dedPart,color:"#F472B6",key:"partner"}].map(e=>(
                  <div key={e.key} style={{background:"#FDFAF5",borderRadius:14,padding:"20px 22px",border:`1px solid ${e.color}33`}}>
                    ...
                    {[
                      {l:"Gross Salary",v:e.salary,c:"#1C1917",pct:100,deduct:false},
                      {l:"SSS",v:e.ded.sss,c:"#4F54D4",pct:(e.ded.sss/e.salary)*100,deduct:true},
                      {l:"PhilHealth",v:e.ded.phic,c:"#F472B6",pct:(e.ded.phic/e.salary)*100,deduct:true},
                      {l:"Pag-IBIG",v:e.ded.hdmf,c:"#B45309",pct:(e.ded.hdmf/e.salary)*100,deduct:true},
                      {l:"W/Tax (BIR)",v:e.ded.tax,c:"#D94F4F",pct:(e.ded.tax/e.salary)*100,deduct:true},
                    ].map((row,i)=>(
                      <div key={row.l} style={{marginBottom:8}}>
                        <div style={{display:"flex",justifyContent:"space-between",marginBottom:3}}>
                          <span style={{fontSize:11,color:i===0?"#1C1917":"#57534E"}}>{row.deduct?"−":""} {row.l}</span>
                          <span style={{fontSize:11,fontWeight:600,color:row.c}}>{row.deduct?"−":""}{pesoF(row.v)}</span>
                        </div>
                        <div style={{height:5,background:"#DDD8CE",borderRadius:3,overflow:"hidden"}}>
                          <div style={{height:"100%",width:`${Math.min(row.pct,100)}%`,background:row.c,borderRadius:3,transition:"width .9s"}}/>
                        </div>
                      </div>
                    ))}
                    <div style={{display:"flex",justifyContent:"space-between",padding:"8px 0",borderTop:"1px solid #C8C2B6",marginTop:6}}>
                      <span style={{fontSize:12,fontWeight:700}}>= Net Take-Home</span>
              ...
              <div style={{background:"#FDFAF5",borderRadius:13,padding:"20px 24px",border:"1px solid #DDD8CE",marginBottom:18}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:18}}>
                  <div>
                    <div style={{fontWeight:700,fontSize:14}}>⚙️ Deduction Rates & Formulas</div>
                    <div style={{fontSize:11,color:"#78716C",marginTop:2}}>Edit these if TRAIN Law rates change. All amounts recompute instantly.</div>
                  </div>
                  <Btn variant="ghost" style={{fontSize:11,padding:"5px 12px"}} onClick={()=>setTaxSettings(DEFAULT_TAX_SETTINGS)}>Reset to defaults</Btn>
                </div>

                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
                  {/* SSS */}
                  <div style={{background:"#EDE8DE",borderRadius:10,padding:"14px 16px"}}>
                    <div style={{fontSize:11,fontWeight:700,color:"#4F54D4",marginBottom:12}}>🛡️ SSS (2024 Table)</div>
                    {[
                      {l:"Base Contribution (₱)",k:"baseContrib"},{l:"Step Amount (₱)",k:"stepAmount"},
                      {l:"Min Salary Credit (₱)",k:"minSalary"},{l:"Max Salary Credit (₱)",k:"maxSalary"},
                      {l:"Max Employee Contribution (₱)",k:"maxContrib"},
                    ].map(f=>(
                      <div key={f.k} style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                        <span style={{fontSize:11,color:"#57534E"}}>{f.l}</span>
                        <input type="number" value={taxSettings.sss[f.k]}
                          onChange={e=>setTaxSettings(x=>({...x,sss:{...x.sss,[f.k]:+e.target.value}}))}
                          style={{width:90,background:"#F5F0E8",border:"1px solid #C8C2B6",borderRadius:6,padding:"4px 8px",color:"#1C1917",fontSize:12,outline:"none",fontFamily:"inherit",textAlign:"right"}}/>
                      </div>
                    ))}
                  </div>
                  ...
                  {/* PhilHealth */}
                  <div style={{background:"#EDE8DE",borderRadius:10,padding:"14px 16px"}}>
                    <div style={{fontSize:11,fontWeight:700,color:"#F472B6",marginBottom:12}}>🏥 PhilHealth (PHIC)</div>
                    {[
                      {l:"Premium Rate (total %)",k:"rate",pct:true},
                      {l:"Min Salary Credit (₱)",k:"minSalary"},{l:"Max Salary Credit (₱)",k:"maxSalary"},
                    ].map(f=>(
                      <div key={f.k} style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                        <span style={{fontSize:11,color:"#57534E"}}>{f.l}</span>
                        <div style={{display:"flex",alignItems:"center",gap:3}}>
                          <input type="number" step="0.01"
                            value={f.pct?pct2(taxSettings.phic[f.k]):taxSettings.phic[f.k]}
                            onChange={e=>setTaxSettings(x=>({...x,phic:{...x.phic,[f.k]:f.pct?dec2(+e.target.value):+e.target.value}}))}
                            style={{width:70,background:"#F5F0E8",border:"1px solid #C8C2B6",borderRadius:6,padding:"4px 8px",color:"#1C1917",fontSize:12,outline:"none",fontFamily:"inherit",textAlign:"right"}}/>
                          {f.pct&&<span style={{fontSize:11,color:"#78716C"}}>%</span>}
                        </div>
                      </div>
                    ))}
                    <div style={{fontSize:10,color:"#78716C",marginTop:6}}>Employee pays 50% of total rate.</div>
                  </div>

                  {/* Pag-IBIG */}
                  <div style={{background:"#EDE8DE",borderRadius:10,padding:"14px 16px"}}>
                    <div style={{fontSize:11,fontWeight:700,color:"#B45309",marginBottom:12}}>🏠 Pag-IBIG (HDMF)</div>
                    {[
                      {l:"Rate if salary ≤ threshold (%)",k:"rateLow",pct:true},
                      {l:"Rate if salary > threshold (%)",k:"rateHigh",pct:true},
                      {l:"Threshold salary (₱)",k:"threshold"},{l:"Max contribution (₱)",k:"maxContrib"},
                    ].map(f=>(
                      <div key={f.k} style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                        <span style={{fontSize:11,color:"#57534E"}}>{f.l}</span>
                        <div style={{display:"flex",alignItems:"center",gap:3}}>
                          <input type="number" step="0.01"
                            value={f.pct?pct2(taxSettings.hdmf[f.k]):taxSettings.hdmf[f.k]}
                            onChange={e=>setTaxSettings(x=>({...x,hdmf:{...x.hdmf,[f.k]:f.pct?dec2(+e.target.value):+e.target.value}}))}
                            style={{width:70,background:"#F5F0E8",border:"1px solid #C8C2B6",borderRadius:6,padding:"4px 8px",color:"#1C1917",fontSize:12,outline:"none",fontFamily:"inherit",textAlign:"right"}}/>
                          {f.pct&&<span style={{fontSize:11,color:"#78716C"}}>%</span>}
                        </div>
                      </div>
                  ...
                  {/* BIR Tax Brackets */}
                  <div style={{background:"#EDE8DE",borderRadius:10,padding:"14px 16px"}}>
                    <div style={{fontSize:11,fontWeight:700,color:"#D94F4F",marginBottom:12}}>📄 BIR Tax Brackets (Monthly)</div>
                    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:4,marginBottom:6}}>
                      <span style={{fontSize:9,color:"#78716C",textTransform:"uppercase"}}>Over</span>
                      <span style={{fontSize:9,color:"#78716C",textTransform:"uppercase"}}>Base Tax</span>
                      <span style={{fontSize:9,color:"#78716C",textTransform:"uppercase"}}>Rate %</span>
                    </div>
                    {taxSettings.bir.map((br,i)=>(
                      <div key={i} style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:4,marginBottom:5,alignItems:"center"}}>
                        <span style={{fontSize:10,color:"#57534E"}}>{br.min===0?"₱0":`₱${br.min.toLocaleString()}`}</span>
                        <input type="number" value={br.base}
                          onChange={e=>setTaxSettings(x=>({...x,bir:x.bir.map((b2,j)=>j===i?{...b2,base:+e.target.value}:b2)}))}
                          style={{background:"#F5F0E8",border:"1px solid #C8C2B6",borderRadius:5,padding:"3px 6px",color:"#1C1917",fontSize:11,outline:"none",fontFamily:"inherit",width:"100%"}}/>
                        <div style={{display:"flex",alignItems:"center",gap:2}}>
                          <input type="number" step="1" value={Math.round(br.rate*100)}
                            onChange={e=>setTaxSettings(x=>({...x,bir:x.bir.map((b2,j)=>j===i?{...b2,rate:+e.target.value/100}:b2)}))}
                            style={{background:"#F5F0E8",border:"1px solid #C8C2B6",borderRadius:5,padding:"3px 6px",color:"#1C1917",fontSize:11,outline:"none",fontFamily:"inherit",width:"100%"}}/>
              ...
              <div style={{background:"#FDFAF5",borderRadius:13,padding:"18px 22px",border:"1px solid #DDD8CE",marginBottom:18}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
                  ...
                  <Btn variant="ghost" style={{fontSize:12,padding:"6px 14px"}} onClick={()=>setModal("addSource")}>+ Add Source</Btn>
                </div>
                {incomeSources.length===0&&(
                  <div style={{background:"#EDE8DE",borderRadius:10,padding:"16px 18px",textAlign:"center",color:"#78716C",fontSize:13,border:"1px dashed #C8C2B6"}}>No other income sources yet.</div>
                )}
                <div style={{display:"grid",gap:8}}>
                  {incomeSources.map(src=>{
                    const it=INCOME_TYPES.find(t=>t.v===src.type)||{l:src.type};
                    const rxMatch=receivedIncome.find(rx=>rx.type===src.type&&(rx.earner===src.earner||src.earner==="Joint"||rx.earner==="Joint")&&Math.abs(rx.amount-src.expectedAmt)/(src.expectedAmt||1)<=0.10);
                    return(
                      <div key={src.id} style={{display:"flex",alignItems:"center",gap:12,padding:"13px 16px",background:"#EDE8DE",borderRadius:10,border:"1px solid",borderColor:rxMatch?"#0D9E8055":"#DDD8CE"}}>
                        <div style={{flex:1,minWidth:0}}>
                          ...
                          {rxMatch&&<div style={{fontSize:10,color:"#0D9E80",marginTop:3}}>Received {pesoF(rxMatch.amount)} on {rxMatch.date}</div>}
                        </div>
                        <Tag color={eColor(src.earner)}>{src.earner}</Tag>
                ...
                {incomeSources.length>0&&(
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",paddingTop:12,borderTop:"1px solid #DDD8CE",marginTop:8}}>
                    <span style={{fontSize:12,color:"#78716C"}}>{incomeSources.filter(s=>receivedIncome.find(rx=>rx.type===s.type&&(rx.earner===s.earner||s.earner==="Joint")&&Math.abs(rx.amount-s.expectedAmt)/(s.expectedAmt||1)<=0.10)).length} of {incomeSources.length} received</span>
                    <span style={{fontSize:13,fontWeight:700,color:"#0D9E80"}}>Total expected: {pesoF(totalExpOther)}</span>
                  </div>
                )}
              </div>
            </div>
      ...
      {modal==="addIncome"&&(
        <Modal title="Record Income Received" onClose={closeModal}>
          <div style={{background:"#0D9E8012",border:"1px solid #0D9E8030",borderRadius:8,padding:"9px 13px",fontSize:12,color:"#0D6B55",marginBottom:14}}>💡 Enter the actual amount you received.</div>
          <Inp label="Description" placeholder="e.g. June Salary" value={form.desc||""} onChange={e=>ff("desc",e.target.value)}/>
          ...
          <MatchPreview amount={form.amount} type={form.type} earner={form.earner} sources={incomeSources}/>
      ...
      {modal==="addSource"&&(
        <Modal title="Add Expected Income Source" onClose={closeModal}>
          <div style={{background:"#4F54D412",border:"1px solid #4F54D430",borderRadius:8,padding:"9px 13px",fontSize:12,color:"#2D3183",marginBottom:14}}>📋 This is a <strong>planned</strong> source. Log actual received amounts in Transactions.</div>
        ...
        <Modal title="Record Expense" onClose={closeModal}>
          ...
          {cards.find(c=>c.name===form.via)&&(
            <div style={{background:"#4F54D418",border:"1px solid #4F54D444",borderRadius:8,padding:"9px 13px",fontSize:12,color:"#4F54D4",marginBottom:12}}>
              💳 This will be added to <strong>{form.via}</strong>'s outstanding balance.
            </div>
          )}
          <div style={{display:"flex",gap:8,justifyContent:"flex-end",marginTop:8}}>
            <Btn variant="ghost" onClick={closeModal}>Cancel</Btn>
      ...
      {modal==="addBudget"&&(
        <Modal title="Add Budget Category" onClose={closeModal}>
          <Sel label="Category" value={form.cat||"other"} onChange={e=>ff("cat",e.target.value)}>
            {CATS.map(c=><option key={c.v} value={c.v}>{c.i} {c.l}</option>)}
          </Sel>
          <Inp label="Monthly Limit (₱)" type="number" placeholder="0.00" value={form.limit||""} onChange={e=>ff("limit",e.target.value)}/>
          <div style={{display:"flex",gap:8,justifyContent:"flex-end",marginTop:8}}>
            <Btn variant="ghost" onClick={closeModal}>Cancel</Btn>
            <Btn onClick={addBudget}>Add Category</Btn>
          </div>
        </Modal>
      )}
    </>
